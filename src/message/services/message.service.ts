...
export class MessageService {
    ...
    public async getConservation(req:LimitConservationReqDto):Promise<MessageResDto[]>{
        //check ex
        const [sender, group]:[Profile, Group] =  await Promise.all([
            this.userRepository.findOneById(req.sender_id),
            req.group_id ? this.groupRepository.isMemberInGroup(req.group_id, req.sender_id):null
        ])
        if (!sender) throw new BadRequestException()
        else if (req.group_id && !group) throw new BadRequestException()
    ...
    public async addNewGroupMessage(createMessage: CreateGroupMessageReqDto):Promise<GroupMessageResDto> {
        //check ex
        const [sender,group, reply]: [Profile, Group, MessageEntity] = await Promise.all([
            this.userRepository.findOneById(createMessage.sender),
